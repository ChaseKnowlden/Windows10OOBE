    ...
    class accountSetupCategoryViewModel {
        ...
        waitForSyncSessionsInitiationAsync() {
            if (this.syncSyncSessionsShouldStart) {
                this.sessionUtilities.logInfoEvent("BootstrapStatus: Start background sync sessions for Account Setup.");
        ...
        async waitForAadRegistrationAsync() {
            ...
            this.sessionUtilities.logInfoEvent(`BootstrapStatus: Configuring AAD device registration timeout for ${aadRegistrationWaitTimeInMilliseconds}ms`);
            ...
            this.deviceRegistrationTaskScheduler = setInterval(
                () => {
                    this.sessionUtilities.enrollmentApis.forceRunDeviceRegistrationScheduledTaskAsync().then(
                        () => {
                            this.sessionUtilities.logInfoEvent("BootstrapStatus: Starting AAD device registration task succeeded");
                        },

                        (e) => {
                            this.sessionUtilities.logErrorEvent("BootstrapStatus: Starting AAD device registration task failed (non-fatal)", e);
                        });
                },
            ...
            let aadRegistrationWaitPromise = new WinJS.Promise(
                ...
                (completeDispatch, errorDispatch, progressDispatch) => {
                    ...
                    this.aadRegistrationListener = (hresult) => {
                        this.sessionUtilities.logInfoEvent(`BootstrapStatus: AAD device registration task completed with hresult ${hresult.target}`);
            ...
            let aadRegistrationTimeoutPromise = WinJS.Promise.timeout(aadRegistrationWaitTimeInMilliseconds).then(() => {
                this.sessionUtilities.logInfoEvent("BootstrapStatus: AAD registration timed out");
            });
        ...
        sendResultsToMdmServerAsync() {
            // Best effort
            try {
                this.sessionUtilities.logInfoEvent("BootstrapStatus: Account setup category sending success results to MDM server.");

                this.sessionUtilities.enrollmentApis.updateServerWithResult(true, this.sessionUtilities.runningInOobe());

                this.sessionUtilities.logInfoEvent("BootstrapStatus: Account setup category sent success results to MDM server.");
            } catch (e) {
